...
function claimListJSON(query,options) {
  return claimListAdvanced("https://ratty.pythonanywhere.com/for_nerd_bob", null, options, includeXPath_, defaultTransform_);
}
...
function playerListAdvanced(url, query, options, includeFunc, transformFunc) {
  ...
  return playerparseJSONObject_(object, query, options, includeFunc, transformFunc);
}
...
function claimListAdvanced(url, query, options, includeFunc, transformFunc) {
  var jsondata = UrlFetchApp.fetch(url);
  var object = JSON.parse(jsondata.getContentText());
  return claimparseJSONObject_(object, query, options, includeFunc, transformFunc);
}

function playerparseJSONObject_(object, query, options, includeFunc, transformFunc) {
...
function claimparseJSONObject_(object, query, options, includeFunc, transformFunc) {
  var headers = new Array();
  var data    = new Array();
  if (query && !Array.isArray(query) && query.toString().indexOf(",") != -1) {
    query = query.toString().split(",");
